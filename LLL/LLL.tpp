...
template <typename TYPE>
int LLL<TYPE>::clear(Node<TYPE> * & head)
{
    if (!head) 
    {
        return true;
...
template <typename TYPE>
int LLL<TYPE>::display(Node<TYPE> * head) const
{
    if (!head)
    {
        return true;
...
template <typename TYPE>
int LLL<TYPE>::remove(Node<TYPE> * & head, const TYPE & target_data)
{
    //Base case: No node
    if (!head) 
    {
    ...
    if (*head == target_data)
    {
        Node<TYPE> * temp = head;
        head = head->get_next();
        temp->get_next() = nullptr; //Unlink before delete to prevent destructor cascading into the live list
        delete temp;
        temp = nullptr;
...
template <typename TYPE>
int LLL<TYPE>::copy(Node<TYPE> * & dest, Node<TYPE> * source)
{
    if (!source)
    {
        dest = nullptr;
...
template <typename TYPE>
int LLL<TYPE>::insert(Node<TYPE> * & head, const TYPE & new_data)
{
    if (!head)
    {
...
template <typename TYPE>
TYPE LLL<TYPE>::retrieve(Node<TYPE> * head, const TYPE & target_data) const
{
    //Base Case: Last Node
